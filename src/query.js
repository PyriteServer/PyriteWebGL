...
class PyriteQuery {
        ...
        this.api.cachedGet(this.versionUrl, 'json').then((r) => {
            ...
            if (r.status == 'OK') {
                ...
                for (let i = 0; i < detailLevels.length; i++) {
                    const dl = new PyriteDetailLevel(this, this.loader.pyrite.cubeDetectorGroup, this.loader.pyrite.modelMeshGroup);
                    dl.Name = detailLevels[i].name;
                    const value = parseInt(dl.Name.substring(1));
                    dl.Value = value;
                ...
                if (this.config.useworldbounds) {
                    ...
                    const min = new THREE.Vector3(highestlod.modelBounds.min.x + lowestlod.worldCubeScale.x / 2,
                        highestlod.modelBounds.min.z + lowestlod.worldCubeScale.z / 4,
                        highestlod.modelBounds.min.y + lowestlod.worldCubeScale.y / 2);
                    const max = new THREE.Vector3(highestlod.modelBounds.max.x - lowestlod.worldCubeScale.x / 2,
    ...
    loadAll(callback, thingsThatNeedElevation) {
        this.loadMetadata(() => {
            ...
            vals.forEach((dl, dlIndex) => {
                const maxboundingboxquery = `${dl.WorldBoundsMin.x},${dl.WorldBoundsMin.y},${dl.WorldBoundsMin.z}/${dl.WorldBoundsMax.x},${dl.WorldBoundsMax.y},${dl.WorldBoundsMax.z}`;
                ...
                this.api.cachedGet(cubesUrl, 'json').then((r) => {
                    ...
                    for (let i = 0; i < cubes.length; i++) {
                        ...
                        if (dl.downgradeLevel) {
                            let factor = this.getNextCubeFactor(dl.downgradeLevel.Value - 1);
                            let downgradeParentIndex = `${Math.floor(cubeContainer.cube.x / factor.x)}_${Math.floor(cubeContainer.cube.y / factor.y)}_${Math.floor(cubeContainer.cube.z / factor.z)}`;
                        ...
                        const min = new THREE.Vector3(cubeContainer.cube.x, cubeContainer.cube.y, cubeContainer.cube.z);
                        ...
                        cubeContainer.textureUrl = this.GetTexturePath(dl.Name, cubeContainer.textureCoords.x, cubeContainer.textureCoords.y);
                        cubeContainer.geometryUrl = this.GetModelPath(dl.Name, cubeContainer.cube.x, cubeContainer.cube.y, cubeContainer.cube.z);
                    ...
                    let clonedThingsThatNeedElevation = thingsThatNeedElevation.slice(0);
                    const halfWorldCubeScale = dl.WorldCubeScale.x / 2;

                    if (dlIndex === 0) {
                        for (let i = 0; i < clonedThingsThatNeedElevation.length; i++) {
                            // console.log(clonedThingsThatNeedElevation[i]);
                            if (this.loader.pyrite.modelConfig.markerYOffsets) {
                                let translateYValue = this.loader.pyrite.modelConfig.markerYOffsets[clonedThingsThatNeedElevation[i].TeeId || clonedThingsThatNeedElevation[i].HoleId];
                                if (translateYValue) {
                                    // console.log(`Fixing ${clonedThingsThatNeedElevation[i].TeeId || clonedThingsThatNeedElevation[i].HoleId} to ${translateYValue}`);
                    ...
                    //         if ((cubeContainer.cube.correctedWorldCoords.x - halfWorldCubeScale < clonedThingsThatNeedElevation[i].marker.position.x) &&
                    //             (cubeContainer.cube.correctedWorldCoords.x + halfWorldCubeScale > clonedThingsThatNeedElevation[i].marker.position.x)) {
                    //             if ((cubeContainer.cube.correctedWorldCoords.z - halfWorldCubeScale < clonedThingsThatNeedElevation[i].marker.position.z) &&
                    //                 (cubeContainer.cube.correctedWorldCoords.z + halfWorldCubeScale > clonedThingsThatNeedElevation[i].marker.position.z)) {
                    ...
                    //                 cubeContainer.nearMarkers.push(clonedThingsThatNeedElevation[i]);
                    //                 clonedThingsThatNeedElevation[i].nearCubes.push(cubeContainer);

                    //                 if (dlIndex === 0) {
                    //                     if (clonedThingsThatNeedElevation[i].marker.position.y > cubeContainer.cube.correctedWorldCoords.y + halfWorldCubeScale) {
                    //                         clonedThingsThatNeedElevation[i].marker.position.y = cubeContainer.cube.correctedWorldCoords.y + halfWorldCubeScale * 1.5;
                    //                         // let translateYValue = this.loader.pyrite.modelConfig.markerYOffsets[clonedThingsThatNeedElevation[i].TeeId || clonedThingsThatNeedElevation[i].HoleId];
                    //                         // if (translateYValue) {
                    //                         //     clonedThingsThatNeedElevation[i].marker.position.y = translateYValue;
                        ...
                        vals.forEach((detailLevel) => {
                            if (detailLevel.childrenMap) {
                                detailLevel.Cubes.forEach((cube) => {
                                    var index = `${cube.cube.x}_${cube.cube.y}_${cube.cube.z}`;
                                    if (detailLevel.childrenMap[index]) {
                                        cube.upgradeChildren = detailLevel.childrenMap[index];
                                        cube.upgradeChildren.forEach((child) => child.downgradeParent = cube);
    ...
    GetModelPath(lod, x, y, z) {
        ...
        let host = projectConfig.pyriteHosts[x % 2];
